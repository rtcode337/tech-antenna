...
**設定の中の並びはサイドバーと同じ順**(ホーム → トレンド → 興味トピック → 定番)で
見出しを立てる —— 8 個を平らに並べると「どの画面のものを集めるボタンか」が読めない。
かつて興味トピック側(論文・イベント・書籍)だけ各ページに置いていたのは、トピックの
選択と地続きで示せるほうが分かると考えたため。**その説明は導線の文と設定側の注記に移した**
(「選んでいないと 1 件も集まらない」)。
**語彙の整備(`/topics` の「話題度を取り直す」・`/tags` の「タグを仕分けなおす」)は
そのページに残す** —— どちらも設定の下のページで、一覧を読みに来る画面ではないため。
...
| ページ | 見せるもの |
|---|---|
| `/topics` | トピック一覧(ツリー/ランキング)・収集対象の選択・「話題度を取り直す」・**語彙の持ち出し / 取り込み**・未仕分けが溜まっていれば `/tags` への案内 |
| `/tags` | **見かけたタグ全部**を状態ごとに。**仕分けを手で直せる**・「タグを仕分けなおす」(トピックの子ページ) |
| `/tags/<キー>` | タグ1語の詳細。**仕分けの状態・出どころ・件数**と、紐づいた記事・イベント・書籍 |
| `/topics/<タグ>` | その語の**同義語**と**親子ツリー上の位置**、続けて紐づいた記事・イベント・書籍 |

**タグの話は `/tags` にまとめる** —— 仕分けのボタン・状態ごとの一覧・まとめて除外が
同じ画面にあり、「仕分けまちを見る → ゴミを除外する → 押す」がそこで完結する。
...
- `/topics` の一覧は**ツリー(既定)とランキングの2表示**を `?view=ranking` のリンクで
  ...
  **「話題度を取り直す」と「ファイルで持ち出す / 取り込む」はページの一番下**(この順)——
  毎回押すものではなく、一覧より先に目に入ると「まず押すもの」に見えてしまう。
  「話題度を取り直す」の説明(手順・話題度の算出方法)は **`?` を押して出るチップ**
  (`popover`。一覧の説明チップと同じ流儀だが、こちらは PC でも出す)にまとめ、
  **副作用(仕分けまちが増える)だけはチップに隠さずボタンの右に書く** ——
  押す前に知っておくべき話のため。**話題度では足切りしない**(0 でも消さない)——
...
規則を変えたら `/topics` の「タグを仕分けなおす」を実行する(仕分けが毎回
