...
**すべてのジョブは画面のボタンから手動で実行できる**(置き場は `/settings`。
...
| 軸 | 親 | 下に付くもの | 何で集めるか |
|---|---|---|---|
| 外で何が話題か | `/recent` トレンド | ニュース / 記事 / **話題の論文**(+ ページ内に「本になっているテーマ」) | RSS の巡回と HF Daily Papers、NDL サーチの新刊(**どれも選択に依存しない**) |
| 自分の興味を深掘る | `/interests` 興味トピック | 書籍 / イベント / 論文 | **選んだトピックを検索語**にした検索(一覧も選んだトピック+配下だけ) |
| 薦められ続けるもの | `/classics` 定番 | 書籍(`/classics/books`) | 「読むべき技術書」記事の推薦(**選択に依存しない**) |
...
- **旧 URL からのリダイレクトは置いていない。** 個人運用でブックマークの数も知れているため、
  経路を2つ抱えるより 404 になるほうがよいと判断した
...
画面は**親子の2段**で、**並びで役割を分ける**。`/recent`「トレンド」(親)が一望のハブで、
...
トレンド(`/recent`)の最後に、**最近出た本・ムックのタイトルから数えたテーマ**を出す。
...
`/settings` 自体は**ジョブのボタンと、子ページ(トピック・主催者・外部連携)への入口**を持つ。
...
- **表示と強調は配下込み**(`TopicCatalog.ExpandWithDescendants`。読む側で毎回広げる)。
  ...
  **興味トピック配下のページ(`/interests/books`・`/interests/events`・`/interests/papers`)は「選んだトピック+配下」の
  ...
  トレンド側(`/recent`・`/recent/news`・`/recent/articles`・`/recent/papers`)と定番
...
- 定期生成は `/settings` の「定期実行」で時刻とチェックを設定する(既定は走らない)。
  **材料が揃ってから走るよう、順番の最後に置いてある**。手動ボタンも `/settings`。
