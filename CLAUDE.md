...
| 軸 | 親 | 下に付くもの | 何で集めるか |
|---|---|---|---|
| 外で何が話題か | `/recent` 直近動向 | ニュース / 記事 / **話題の論文** | RSS の巡回と HF Daily Papers(**選択に依存しない**) |
...
- **話題の論文**(`ArticleKind.TrendingPaper`。Hugging Face Daily Papers)——
  トピックの選択に依存しないので、収集は**記事の収集に含める**(`IArticleSource`)。
  画面は直近動向の下(`/trending-papers`)。**日本語の論文は入らない** ——
...
画面は**親子の2段**で、**並びで役割を分ける**。`/recent`「直近動向」(親)が一望のハブで、
...
- **論文の upvote 数(`Article.UpvoteCount`)は、はてブ数とは別の列**にした
  ...
  並びに使うのは直近動向(話題度の高い順)だけで、`/trending-papers` は新着順
...
- **種別ページ(ニュース・記事・話題の論文)の並びは変えない**(新着順)。話題度は
  ...
  直近動向ではカードの面にも同じ段階が出る(`ArticleCard` の `EmphasizeByPopularity`)。
  **話題度で並べるのは直近動向だけ**
