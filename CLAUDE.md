...
サイドバーは**同じ高さに4本**。ホーム(入口)のあと、**2つの軸**が並び、最後が設定:

| 軸 | 親 | 下に付くもの | 何で集めるか |
|---|---|---|---|
| 外で何が話題か | `/recent` トレンド | ニュース / 記事 / **話題の論文** | RSS の巡回と HF Daily Papers(**選択に依存しない**) |
| 自分の興味を深掘る | `/interests` 興味トピック | 書籍 / イベント / 論文 | **選んだトピックを検索語**にした検索 |
...
**この2軸の分け方が画面構成の背骨。** 同じ「論文」でも探し方の軸が違うので別のページに置く
(`ArticleKind.TrendingPaper` と `ArticleKind.Paper`)。**2軸はホームの子ではなく同列に置く**
—— 子に入れると、対等な2つの入口だと見えない。設定の下はトピック/外部連携で、
...
(記事・書籍など)でまとめる。どちらの軸で使うかは `Integration.Axis`
(`IntegrationAxis` は `[Flags]`)で持ち、**両方で使うものは両方の節に出す** ——
「トレンドが動かない」と「興味トピックが動かない」で見に来る場所が違うので、片方だけに
置くともう片方から辿れない。今のところ両方に出るのは話題度の材料(Qiita トレンド・
はてブ人気エントリー)と LLM(Claude Code・Anthropic API)。
...
- `/topics` の一覧は**ツリー(既定)とランキングの2表示**を `?view=ranking` のリンクで
  ...
  **副作用(仕分けまちが増える)だけはチップに隠さずボタンの右に書く** ——
  ...
  親を選べば配下も収集対象になるので、配下が見えていないと何を選んだのか分からない。
