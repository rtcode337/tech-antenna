...
| ページ | 見せるもの |
|---|---|
| `/topics` | トピック一覧(ツリー/ランキング)・収集対象の選択・**語彙の持ち出し / 取り込み**・未仕分けが溜まっていれば `/tags` への案内 |
...
`/topics` 側には「未仕分けのタグが N 語あります」の案内リンクだけを置く
...
- **表示と強調は配下込み**(`TopicCatalog.ExpandWithDescendants`。読む側で毎回広げる)。
  ...
  **興味トピック配下のページ(`/books`・`/events`・`/papers`)は「選んだトピック+配下」の
  ...
  トレンド側(`/recent`・`/news`・`/articles`)は絞らず★とタグの色で強調する
