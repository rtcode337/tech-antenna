...
語彙に入っていないタグは**一覧から外すが、画面からは消さない**(`/topics` 下部の「一覧に
出していない語」)。実データではタグ 1400 語のうち語彙は 355 語で、混ぜるとツリーの構造が
埋まってしまう。一方で消してしまうと、誤判定や仕分けの遅れに気づけない。

**`Tags.Status` でそのまま 4 つに分かれる**(以前は 3 テーブルの突き合わせで導出していた):

- **仕分けまち**(`Pending`) —— LLM にまだ聞いていない
- **LLM が判断できなかった**(`Unresolved`) —— `RetryAfter` を過ぎたら自動で聞き直す
- **技術用語でないと判定された**(`NotTopic`) —— ここに見えていないと誤判定を直す機会が無い
- **別名に吸収された**(`Alias`) —— 寄せ先も並べて出す(件数は寄せ先へ合算される)

**選ぶ対象ではないのでバッジで出す**(行 + チェックボックスにしない)。選択は語彙に
対して行う —— タグに `IsSelected` は無い。
