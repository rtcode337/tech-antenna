...
- **対象は `TopicCandidateFinder` が保存済みデータだけから決める。**
  画面の「次の再編成で LLM に聞く語」と再編成の入力が<b>同じ 1 か所</b>から出るので、
  押す前に何語流れるかが分かる。材料は 2 つ:
  ① **集めたデータのタグ**(3 回以上付いた語。`MinCount`。1〜2 件の語は誤記や一過性のタグが
  多く、枠を使ってまで整理する価値が無いので平置きのまま残る)
  ② **前回までのトレンドで現れた語**(トピックの行に話題度が付いている未分類の語。件数の
  下限は掛けない —— 手元のデータには 1 件も無いのが普通で、外で話題の新語こそ入れたい)
- **その回のトレンドで見つかった語は、その回では聞かない。** 行として残し、次の回の対象にする ——
...
- 行にはトピック名のほかに**記事の件数**(`StoredTopic.ArticleCount`。0 のときは出さない ——
...
語彙に入っていない語は**一覧から外すが、画面からは消さない**(`/topics` 下部の「一覧に
出していない語」)。実データでは 1070 件のうち 885 件が平置きで、一覧に混ぜると
ツリーの構造が埋まってしまう。一方で消してしまうと、誤判定や分類の遅れに気づけない。

状態ごとに 4 つに分けて出す。**なぜ一覧に無いのかが分かる分け方**にしてある:

- **まだ分類していない** —— LLM にまだ聞いていない語(分類は 1 回 300 語まで、
  タグとして 3 回以上付いた語が対象)
- **LLM が判断できなかった** —— `Unknown`。1 週間おいて自動で聞き直す
- **技術用語でないと判定された** —— `Skip`。**トピック一覧の行ごと消しているので、
  分類の記録(`ITopicClassificationStore`)から拾って出す**。ここに見えていないと
  誤判定を直す機会が無い(直し方は `topic-catalog.json` に書く)
- **別名に吸収された** —— `Alias`。行は残るが件数は寄せ先に合算されている

**選ぶ対象ではないのでバッジで出す**(行 + チェックボックスにしない)。選択は語彙に
入った語に対して行う —— 選択済みの語は状態にかかわらず一覧側に残る。
