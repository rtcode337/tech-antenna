...
- `/topics` の一覧は**ツリー(既定)とランキングの2表示**を `?view=ranking` のリンクで
  ...
  面とつなげる)。タブの下の1行に選択の説明(左)と検索(右)を並べ、一覧の直上に
  「ファイルで持ち出す / 取り込む」の折りたたみを置く。
...
- **別名にする操作だけ一段進める**(`?alias=<キー>` で寄せ先を選ぶフォームを出す)。
  行ごとに選択肢を置くと 1000 行のページに `select` が 1000 個並ぶ。寄せ先は `datalist` で
  打ちながら絞れる
