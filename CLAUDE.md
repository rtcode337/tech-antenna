...
直近動向の下にさらにニュース/記事/論文が1段下がってぶら下がる。設定の下にトピック/外部連携で、
トピックの詳細 `/topics/<タグ>` も設定の側に属する)。
...
`TopicService` がそれを突き合わせてトピックの詳細(`/topics/<タグ>`)に出す。
...
| ページ | 見せるもの |
|---|---|
| `/topics` | トピック一覧(ツリー/ランキング)・収集対象の選択・「トピックを再編成」 |
| `/topics/<タグ>` | その語の**同義語**と**親子ツリー上の位置**、続けて紐づいた記事・イベント・書籍 |

`/settings` 自体は**子ページ(トピック・外部連携)への入口だけ**を持つ。1ページに詰めると
長い一覧と表が混ざって目的の操作に辿り着けないため。
...
- **語彙の姿は `TopicCatalog.StructureOf` が組む**(`TopicStructure`)。ストアではなくカタログを
  見るのは、**語彙の出どころを1つにするため** —— `StoredTopic.Parent` はカタログから写した値で、
  同義語はそもそもストアに無い。LLM の分類は起動時と再編成時にカタログへ合成済み
...
何を集めるかは `/topics`(設定 → トピック)の**行頭のチェックボックス**で決める(概要は
README「トピック収集」)。保存は `UpdateSelectionAsync` で、**一覧に出ている分で丸ごと
置き換える**(チェックの外れたものは POST に乗らないため)。

**親を選んだら配下のトピックも収集対象にする**(`TopicCatalog.ExpandWithDescendants`)。
「AI を集めたい」と言っているのに `LLM` や `RAG` のイベント・書籍が集まらないのは、選んだ側の
期待と合わないため。

- **広げるのはサーバー側(保存時)**。`wwwroot/topic-tree.js` も親のチェックを配下へ広げるが、
  あれは「保存したらこうなる」を押した瞬間に見せる上乗せで、**JS が動かなくても結果は同じ**
- **子だけを外すことはできない**(親を選んでいる限り、保存のたびに広がる)。外したいときは
  親のチェックを外して選び直す —— 「部分木を選ぶ」以上に細かい状態を持たせると、
  画面のチェックと保存結果が食い違って読めなくなる
- **イベントと書籍はトピック 1 つごとに外部へ問い合わせる**ので、大きな親を選ぶと
  リクエスト数も配下のぶんだけ増える。画面にもそう書いてある
...
- `/topics` の一覧は**ツリー(既定)とランキングの2表示**を `?view=ranking` のリンクで
...
- **既にカタログにあって説明だけ無い語は、再編成の中で埋める**(`DescribeMissingAsync`)。
...
規則を変えたら `/topics` の「トピックを再編成」を実行する(再編成が毎回
