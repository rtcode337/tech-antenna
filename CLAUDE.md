...
| タグの仕分け直し(未知語の LLM 分類) | (定期実行しない) | — | `/topics`・`/tags` |
...
| ページ | 見せるもの |
|---|---|
| `/topics` | トピック一覧(ツリー/ランキング)・収集対象の選択・「話題度を取り直す」/「タグを仕分けなおす」・次に LLM へ聞く語・**語彙の持ち出し / 取り込み** |
| `/tags` | **見かけたタグ全部**を状態ごとに。**仕分けを手で直せる**(トピックの子ページ) |
| `/topics/<タグ>` | その語の**同義語**と**親子ツリー上の位置**、続けて紐づいた記事・イベント・書籍 |

**「次に聞く語」は `/topics` に置き、状態ごとの一覧は `/tags` に置く。** どちらもタグの話だが、
前者は<b>語彙に加えようとしている語</b>の話(トピックの側)で、後者は<b>タグそのもの</b>の話。
...
入り、画面の「次の仕分けで LLM に聞く語」にも出る)—— 候補集め(語彙)と話題度(鮮度)は
...
- **画面には「次に聞く語」と「前回聞いた語」の両方を出す。** 次に聞く語は上の候補そのままで、
  **枠(300 語)に収まらない分は「次回以降に回る語」として分けて出す**。
  前回聞いた語は `JobRunner` 側の記録を優先し、無ければ DB から復元する
  (**同じ実行で付けた分類は `ClassifiedAt` が揃う**ので、最新の日時の束が前回の結果になる)
- **対象は `ITagStore.GetPendingAsync` が決める**(状態が `Pending`、または `RetryAfter` を
  過ぎた `Unresolved`)。画面の「次の仕分けで LLM に聞く語」と仕分けの入力が<b>同じ 1 か所</b>
  から出るので、押す前に何語流れるかが分かる
...
- `/topics` の一覧は**ツリー(既定)とランキングの2表示**を `?view=ranking` のリンクで
  切り替える(静的 SSR のまま)。**話題度では足切りしない**(0 でも消さない)——
...
一覧の上に検索ボックスを置いてある。**GET のフォーム**(`?q=`)なので静的 SSR のまま動く。
