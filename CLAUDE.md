...
測るのはカードの強調と同じ規則)で数件ずつ出す —— ここは「何が話題か」に答えるページ。
全量は子ページ(`/news`・`/articles`・`/trending-papers`、3ルートで
1 コンポーネント `ArticleListPage`)が**新着順**で受け持つ(話題度は並びではなく
カードの強調に出す)—— 同じ一覧が2つの並びを持つと、どちらを見ているのか分からなくなる。カードは共通の
...
- **論文の upvote 数(`Article.UpvoteCount`)は、はてブ数とは別の列**にした
  ...
  並びに使うのはトレンド(話題度の高い順)だけで、`/trending-papers` は新着順
  (かつては子ページも upvote 順だったが、並びの役割分担に合わせて統一した)。
...
- **種別ページ(ニュース・記事・話題の論文)の並びは変えない**(新着順)。話題度は
  目立ち方に出す —— バッジが件数で3段階(10 users 未満 / 10〜99 / 100 以上)、
  トレンドではカードの面にも同じ段階が出る(`ArticleCard` の `EmphasizeByPopularity`)。
  **話題度で並べるのはトレンドだけ**
