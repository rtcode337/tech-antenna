...
| ブックマーク数の補完 | (記事の収集に含まれる) | — | `/articles` |
...
サイドバーは**2階層**(ホームの下に書籍/イベント、記事の下に記事の一覧/ニュース/論文、
設定の下に外部連携)。折りたたみは
...
画面は**親子の2段**。`/articles`(親)が「注目」のハブで、直近 7 日からブックマーク数の
多い記事・ニュースを数件ずつ出す。全量は子ページ(`/articles/list`・`/news`・`/papers`、
3ルートで 1 コンポーネント `ArticleListPage`)が**新着順**で受け持つ。カードは共通の
`Components/ArticleCard.razor`。収集・要約のボタンは親に、翻訳は `/papers` に置く。
...
**人気順の選別はあるが数値は乗らない**(実測)。他ソースの分は、収集の最後に
**はてなブックマーク件数取得 API**(`HatenaBookmarkCounts`。キー不要・50 URL/リクエスト)で
直近 7 日の記事・ニュースをまとめて引き直す。論文は対象外(はてブにほぼ載らない)。

- **null は「未取得」、0 は「ブックマークされていない」**で別物(書籍の `ReviewCount` と同じ)
- **並び順は変えない**(一覧は新着順のまま)。人気は目立ち方に出す —— カードのバッジが
  件数で3段階(10 users 未満 / 10〜99 / 100 以上)、親ページの「注目」の抽出にも使う
- 窓を 7 日にしているのは、窓が無いと古い有名記事が「注目」に居座り続けるため
...
見えなくなるだけなので全部保存し、`/articles` で選択トピックのものを強調している。
...
正常に見え、「タグを再正規化」を押した瞬間にその本のタグが空になる(`OpenBdEnricher` で
実際に踏んだ)。
