...
| トピックの収集(未知語の LLM 分類を含む) | (定期実行しない) | — | `/settings` |
...
カタログに無い語(トレンドや収集元のタグから入ってくる)は、**トピック収集の中で LLM が
分類してツリーへ入れる**。同義語・粒度の違いの判定は語の意味を知らないとできないため、
...
- **Skip も保存する** —— 保存しないと、トピックでない語を毎回 LLM に聞き直して枠を無駄にする。
  一方 **unknown は保存しない**(次回もう一度聞く)。skip は「トピックでないという確信」、
  unknown は「判断できない」で意味が違う —— unknown まで確定させると、LLM の知識に無い
  新語(まさにツリーに入れたい語)が永久に平置きのまま残る。応答に無かった語も同様に次回聞く
- 分類が通ったら**同じ実行の中でタグの再正規化と件数の集計をやり直す**(別名が増えた分を
  保存済みデータへ反映してから一覧を作る)
- **対象は「外部トレンドに現れた語」と「収集済みの記事等に 3 回以上タグとして付いた語」だけ**
  (`MinInventoryForClassification`)。1〜2 件の語は誤記や一過性のタグが多く、
