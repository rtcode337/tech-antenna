...
画面の呼称は**「トレンド」**(かつては「直近動向」)。**URL(`/recent`)と内部名
(`IntegrationAxis.Trending` など)は変えていない** —— 表示名の変更でブックマークや
コードの識別子まで巻き込まない。話題度の材料の「外部トレンド」(Qiita・はてブ)とは別の話。

**この軸の分け方が画面構成の背骨。** 同じ「論文」でも探し方の軸が違うので別のページに置く
(`ArticleKind.TrendingPaper` と `ArticleKind.Paper`)。同じ「書籍」も軸で分かれる
(選んだトピックの検索が `/books`、トピックに寄らない推薦が `/classics/books`)。
**軸はホームの子ではなく同列に置く** —— 子に入れると、対等な入口だと見えない。設定の下はトピック/外部連携で、
トピックの下にさらにタグが1段下がる(トピックの詳細 `/topics/<タグ>` も設定の側)。
...
`/topics`・`/tags` は設定の下のページだが、**ジョブのボタンは1か所**という規則を
...
- **話題の論文**(`ArticleKind.TrendingPaper`。Hugging Face Daily Papers)——
  ...
  画面はトレンドの下(`/trending-papers`)。**日本語の論文は入らない** ——
...
- **トピック検索の論文**(`ArticleKind.Paper`。arXiv / J-STAGE)——
  ...
  `PaperCollectionRunner`)で、画面は興味トピックの下(`/papers`)。
...
全量は子ページ(`/news`・`/articles`・`/trending-papers`、3ルートで
...
**`/papers` にはタブを出さない** —— arXiv / J-STAGE の論文には数値が付かず、
...
- **論文の upvote 数(`Article.UpvoteCount`)は、はてブ数とは別の列**にした
  ...
  トレンドは常に話題度の高い順、`/trending-papers` は新着順が既定でタブから upvote 順にできる。
...
| ページ | 見せるもの |
|---|---|
| `/topics` | トピック一覧(ツリー/ランキング)・収集対象の選択・**語彙の持ち出し / 取り込み** |
| `/tags` | **見かけたタグ全部**を状態ごとに。**仕分けを手で直せる**・まとめて除外(トピックの子ページ) |
| `/tags/<キー>` | タグ1語の詳細。**仕分けの状態・出どころ・件数**と、紐づいた記事・イベント・書籍 |
| `/topics/<タグ>` | その語の**同義語**と**親子ツリー上の位置**、続けて紐づいた記事・イベント・書籍 |

**タグの話は `/tags` にまとめる** —— 状態ごとの一覧とまとめて除外が同じ画面にあり、
「仕分けまちを見る → ゴミを除外する」までがそこで完結する(押すのは設定。行き先は画面に書く)。

**「未仕分けのタグが N 語あります」の案内は設定(ボタンの上)に出す。** かつては
`/topics` に置いていたが、**そこには押すボタンが無い** —— 気づいても何もできない案内に
...
- **対象は `ITagStore.GetPendingAsync` が決める**(状態が `Pending`、または `RetryAfter` を
  過ぎた `Unresolved`)。`/tags` の「仕分けまち」と仕分けの入力が<b>同じ 1 か所</b>から
  出るので、押す前に何が流れるかが分かる(`/topics` の案内の語数も同じところから数える)
- **件数で足切りはしない**(`GetPendingAsync` に下限は無い)。**画面の「仕分けまち」が
  ...
  **収集ボタンを何回押したかで対象が変わる**うえ、落ちた語は仕分けまちに残り続けた。
  ...
  次の回へ回す。**ゴミは `/tags` の「チェックした語を除外する」で LLM に聞かせる前に落とす**
...
- **タグ名は詳細ページ(`/tags/<キー>`)へのリンク。** トピックの詳細と同じ作りで
...
ようにしてある(逆向きにも、バックアップにも使える)。画面は `/topics` の
「ファイルで持ち出す / 取り込む」。
...
- **書き出しは `GET /export/topics.json`**(最小 API)。ダウンロードは GET 1本で足りるので
  Blazor のフォームにはしない。**`/topics/…` の下に置かない**のは、トピックの詳細
  (`/topics/<タグ>`)と紛れないようにするため
- **取り込みは `/topics` の multipart フォーム。** ファイルは
...
- **`/books` のトピックの折りたたみの中では推薦 1 本から強調**(左帯 + 薄い面。記事一覧の
