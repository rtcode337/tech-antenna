...
**`Status` を持つ**(仕分けまち / 昇格 / 別名 / トピック外 / 保留)。語彙になるのは
...
| ページ | 見せるもの |
|---|---|
| `/topics` | トピック一覧(ツリー/ランキング)・収集対象の選択・「トピックを再編成」・次に LLM へ聞く語・**語彙の持ち出し / 取り込み** |
...
- **語彙の姿は `TopicCatalog.StructureOf` が組む**(`TopicStructure`)。ストアではなくカタログを
  ...
  組んだスナップショットで、起動時と再編成のあとに組み直される
...
入り、画面の「次の再編成で LLM に聞く語」にも出る)—— 候補集め(語彙)と話題度(鮮度)は
...
- **トピック外(`NotTopic`)も保存する** —— 保存しないと、トピックでない語を毎回
  LLM に聞き直して枠を無駄にする。
...
- **タグの再正規化は再編成の中で毎回、分類の前と後の2回走る**。数秒で冪等な DB 処理なので
  2 回でよく、それぞれ役目が違う:
...
- **対象は `ITagStore.GetPendingAsync` が決める**(状態が `Pending`、または `RetryAfter` を
  過ぎた `Unresolved`)。画面の「次の再編成で LLM に聞く語」と再編成の入力が<b>同じ 1 か所</b>
  から出るので、押す前に何語流れるかが分かる
- **件数の下限(`MinTagCount` = 3)は「集めたデータに付いた回数」だけに掛ける。**
  1〜2 件の語は誤記や一過性のタグが多く枠を使う価値が無いが、**外部トレンドで見つかった語は
  手元の件数が 0 なのが普通**で、そこで落とすと外で話題の新語が永久に入ってこない
- **再編成は観測より先に仕分ける。** だから**その回のトレンドで見つかった語は次の回に回る**
  —— 実行時に取得した語をその場で足すと、押すまで何語 LLM に流れるか分からない
  (実測で「候補 1 語のはずが 73 語聞いていた」)。1 回分待つ代わりに予測がつく
...
- **トピック外と確定した語は語彙に入らない**(タグとしては `NotTopic` で残る)。
  ニュース・開発 のような一般語が話題度の上位を占めると一覧が読めないため
- **同義のトピックは統合パスが寄せる**(`ITopicMergeAdvisor`。再編成の手順 5)。
...
**残骸の行は再編成が掃除する。** 正規化の規則を変えると、以前のキーの行
(`#生成ai`・`,` など)が幽霊として一覧に残る —— 中身は正しい行に合流済みなのに、
`UpsertAsync` は行を消さないため。再編成は次の 2 種類を `RemoveAsync` で消す
(**選択済みは消さない**):
...
- **トピック外・仕分けまちへ戻すときは語彙の行も消す**(`TopicMerger.DemoteAsync`)。
...
- **運ぶのは語彙(`Topics`)とタグの仕分け(`Tags` の状態)だけ。** 件数・話題度・
  ...
  持ち込むと取り込み先の実データと食い違う(件数は次の再編成が集め直す)
