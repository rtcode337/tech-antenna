...
public static class DigestPrompt
{
    ...
    const string Common =
        "lead は全体の導入1〜2文。items は**材料の数に応じて1〜6項目**(材料が少なければ" +
        "少なくてよい。無理に増やさない)で、各項目は title(見出し1行)と " +
        "body(2〜3文。なぜ押さえるべきかまで書く)。" +
        "**材料が1件でもダイジェストを作る** —— 材料が少ないことを理由に断らない" +
        "(実測: 材料1件のときに「作成できません」と文章で返してきて、生成そのものが失敗した)。" +
        "似た話題の記事は1項目にまとめてよい。" +
