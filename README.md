...
- 一覧の左のチェックボックスが**収集対象の選択**。「選択を保存」で確定する。イベント・書籍は
  選んだトピックを検索語にして問い合わせ、記事は RSS を巡回したうえで選んだタグを含むものだけ
  保存する。**何も選んでいないと記事・イベント・書籍は 1 件も集まらない**
...
- **検索は Google Books、補完は openBD** と役割を分けている。openBD は日本の書誌を無料で
  引けるが**キーワード検索を持たず ISBN 参照専用**なので、検索で得た ISBN で引き直して
  **欠けている項目だけ**を埋める(既に値がある項目は上書きしない)
...
- 検索条件は `langRestrict=ja`(日本語の技術書を拾うため)と `orderBy=newest` で、
  **1 キーワードにつき新しい順に 20 件**。キーワードの間は 2 秒空ける
...
- 一度保存した本の書誌情報は**更新しない**。後から openBD 側の情報が増えても反映されない
...
収集は画面のボタンから実行する(→[トピック収集](#トピック収集))。トピックを 1 つも
選んでいない状態では記事もイベントも集まらないので、まず「設定」で選択を保存する。
