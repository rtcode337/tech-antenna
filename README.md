...
「記事・ニュース」画面(親)の**「記事の収集」**で、RSS のフィードと論文の API をまとめて
1 巡する。親ページは**注目**(直近 7 日でブックマークが多い記事・ニュースを数件ずつ)、
種別ごとの全量は子ページ(記事の一覧 / ニュース / 論文)が**新着順**で受け持つ。

- **人気(はてなブックマーク数)は目立ち方に出す**。カードのバッジが件数で 3 段階に変わる。
  並び順は新着のまま —— 人気順に並べ替えると「今日なにが出たか」を追えなくなる。
